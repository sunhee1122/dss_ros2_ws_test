...
public:
    ...
    pcl::PointCloud<PointType>::Ptr globalMapForVisualization;
    ...
    mapOptimization(const rclcpp::NodeOptions & options) : ParamServer("lio_sam_mapOptimization", options)
    {
        ...
        // Subscribe to initial pose for localization mode
        ...
        auto saveMapService = [this](const std::shared_ptr<rmw_request_id_t> request_header, const std::shared_ptr<dss_lio_sam::srv::SaveMap::Request> req, std::shared_ptr<dss_lio_sam::srv::SaveMap::Response> res) -> void {
            ...
            else if(req->destination[0] == '/') saveMapDirectory = req->destination;  // absolute path
    ...
    void loadGlobalMap()
    {
        ...
        *globalMapSurf = *globalMapSurfDS;
        ...
        RCLCPP_INFO(get_logger(), "Downsampled corner map: %lu points", globalMapCorner->size());
        RCLCPP_INFO(get_logger(), "Downsampled surf map: %lu points", globalMapSurf->size());

        // Build kdtree for localization
        kdtreeCornerFromMap->setInputCloud(globalMapCorner);
        kdtreeSurfFromMap->setInputCloud(globalMapSurf);

        // Store for visualization
        *globalMapForVisualization = *globalMapCorner;
        *globalMapForVisualization += *globalMapSurf;

        RCLCPP_INFO(get_logger(), "Global map loaded successfully!");
    }
    ...
    {
        // Only process in localization mode
        if (!localizationMode) {
        ...
        // Update odometry affines
        incrementalOdometryAffineBack = trans2Affine3f(transformTobeMapped);
        incrementalOdometryAffineFront = incrementalOdometryAffineBack;
        ...
        RCLCPP_INFO(get_logger(), "****************************************************");
        RCLCPP_INFO(get_logger(), "Initial pose set: x=%.2f, y=%.2f, z=%.2f, yaw=%.2f deg",
    ...
    void publishGlobalMap()
    {
        ...
        // In localization mode, publish the pre-loaded global map
        if (localizationMode) {
            publishCloud(pubLaserCloudSurround, globalMapForVisualization, this->now(), mapFrame);
            return;
        }
    ...
    void extractSurroundingKeyFrames()
    {
        ...
        if (localizationMode) {
            // Use the global map for localization
    ...
    {
        // In localization mode, skip the keypose check since we use global map directly
        if (!localizationMode && cloudKeyPoses3D->points.empty())
            return;

        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum)
        {
            // In localization mode, kdtree is already set from loadGlobalMap()
            if (!localizationMode) {
    ...
    {
        // In localization mode, don't save keyframes but still update path for visualization
        if (localizationMode) {
            // Update path for localization mode
            PointTypePose thisPose6D;
    ...
    void publishOdometry()
    {
        ...
        // In localization mode, publish map -> odom transform (odom is at robot position)
        if (localizationMode) {
            tf2::Stamped<tf2::Transform> temp_map_to_odom(t_odom_to_lidar, time_point, mapFrame);
